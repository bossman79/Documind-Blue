...
def _build_model_catalog_ssl_context() -> ssl.SSLContext:
    """
    SSL context for OpenRouter / Models.dev catalog fetches (urllib).

    Corporate TLS inspection often breaks default verification. Without admin
    you can still point to a PEM bundle the user controls:

      MODEL_INFO_SSL_CAFILE=C:\\Users\\you\\corp-root.pem

    Standard variables are also honored if set: SSL_CERT_FILE, REQUESTS_CA_BUNDLE,
    MODEL_INFO_SSL_CAPATH / SSL_CERT_DIR.

    Last resort (insecure): MODEL_INFO_SSL_VERIFY=0|false|off|insecure
    """
    verify_flag = os.getenv("MODEL_INFO_SSL_VERIFY", "").strip().lower()
    if verify_flag in ("0", "false", "no", "off", "insecure"):
        ctx = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
        ctx.check_hostname = False
        ctx.verify_mode = ssl.CERT_NONE
        logger.warning(
            "TLS verification disabled for model catalog fetches "
            "(MODEL_INFO_SSL_VERIFY). Prefer MODEL_INFO_SSL_CAFILE with your org CA PEM."
        )
        return ctx

    ctx = ssl.create_default_context()
    cafile = (
        os.getenv("MODEL_INFO_SSL_CAFILE", "").strip()
        or os.getenv("SSL_CERT_FILE", "").strip()
        or os.getenv("REQUESTS_CA_BUNDLE", "").strip()
    )
    capath = (
        os.getenv("MODEL_INFO_SSL_CAPATH", "").strip()
        or os.getenv("SSL_CERT_DIR", "").strip()
    )
    load_kw: Dict[str, str] = {}
    if cafile:
        if os.path.isfile(cafile):
            load_kw["cafile"] = cafile
        else:
            logger.warning(
                "Model catalog CA file not found (ignored): %s", cafile
            )
    if capath:
        if os.path.isdir(capath):
            load_kw["capath"] = capath
        else:
            logger.warning(
                "Model catalog CA path not found (ignored): %s", capath
            )
    if load_kw:
        ctx.load_verify_locations(**load_kw)
    return ctx
...
class DataSourceAdapter:
    ...
    def _http_get(self, url: str, timeout: int = 30) -> Any:
        ...
        ssl_ctx = _build_model_catalog_ssl_context()
