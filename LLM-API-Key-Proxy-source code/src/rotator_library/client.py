...
from .documind_ollama_policy import (
    DocumindOllamaPolicy,
    build_cred_index_maps,
    headers_from_exception,
    headers_from_litellm_response,
    ollama_cloud_extra_headers_for_index,
    order_credentials_documind,
    parse_documind_request_headers,
)
...
class RotatingClient:
    ...
    def __init__(
        ...
        self._init_documind_ollama_cloud()

    def _init_documind_ollama_cloud(self) -> None:
        self.documind_policy: Optional[DocumindOllamaPolicy] = None
        self._ollama_cloud_cred_to_index: Dict[str, int] = {}
        self._ollama_cloud_ordered_keys: List[str] = []
        self._documind_last_success_index: Optional[int] = None
        self._ollama_proxy_clients: Dict[int, httpx.AsyncClient] = {}
        keys = self.all_credentials.get("ollama_cloud") or []
        if not keys:
            return
        self._ollama_cloud_cred_to_index, self._ollama_cloud_ordered_keys = (
            build_cred_index_maps(keys)
        )
        self.documind_policy = DocumindOllamaPolicy(
            file_path=self.data_dir / "documind_ollama_policy.json"
        )

    def import_documind_legacy_if_present(self) -> None:
        if not getattr(self, "documind_policy", None):
            return
        legacy_root = (os.environ.get("DOCUMIND_DATA_DIR") or "").strip()
        if not legacy_root:
            return
        p = Path(legacy_root)
        if not p.is_dir():
            return
        self.documind_policy.import_legacy_files(
            p / "documind_keys_cache.json",
            p / "documind_quota_usage.json",
        )

    def _get_ollama_cloud_httpx_client(self, cred_index: int) -> Optional[httpx.AsyncClient]:
        proxy_url = (
            os.getenv(f"OLLAMA_CLOUD_HTTP_PROXY_{cred_index}", "") or ""
        ).strip()
        if not proxy_url:
            return None
        if cred_index not in self._ollama_proxy_clients:
            # Do not tie httpx read timeout to global_timeout (acquire+retry budget); long
            # LLM responses need generous read while keeping per-request deadline separate.
            try:
                connect_s = float(os.getenv("TIMEOUT_CONNECT", "30"))
            except ValueError:
                connect_s = 30.0
            try:
                read_s = float(os.getenv("TIMEOUT_READ_NON_STREAMING", "600"))
            except ValueError:
                read_s = 600.0
            self._ollama_proxy_clients[cred_index] = httpx.AsyncClient(
                proxy=proxy_url,
        ...
        return self._ollama_proxy_clients[cred_index]

    def _merge_ollama_cloud_litellm_outbound(
        self, litellm_kwargs: Dict[str, Any], current_cred: str
    ) -> None:
        idx = self._ollama_cloud_cred_to_index.get(current_cred)
        if idx is None:
            return
        extra = ollama_cloud_extra_headers_for_index(idx)
        if extra:
            eh = litellm_kwargs.get("extra_headers") or {}
            litellm_kwargs["extra_headers"] = {**eh, **extra}
        alt = self._get_ollama_cloud_httpx_client(idx)
        if alt is not None:
            litellm_kwargs["client"] = alt

    def _documind_on_success(
        self,
        provider: str,
        current_cred: str,
        response: Any,
        ...
        if provider != "ollama_cloud" or not self.documind_policy:
            return
        idx = self._ollama_cloud_cred_to_index.get(current_cred)
        if idx is None:
            return
        hdrs = headers_from_litellm_response(response)
        self.documind_policy.on_upstream_success(idx, hdrs, document_complete)
        self._documind_last_success_index = idx

    def _documind_on_429(
        self,
        provider: str,
        current_cred: str,
        err: Any,
        ...
        *,
        ...
        if idx is None:
            return
        ce = classified_error
        self.documind_policy.on_upstream_429(
            idx,
            error_message or "",
            headers_from_exception(err),
            retry_after_seconds=ce.retry_after if ce else None,
            error_type=ce.error_type if ce else None,
        )

    async def _apply_provider_rate_limit_cooldown_if_needed(
        self,
        *,
        provider: str,
        ...
        """
        Provider-wide cooldown after 429 makes sense for a single key (nothing to rotate).
        With multiple keys, only the failing key should back off (usage_manager); do not
        pause the whole provider or the next iteration cannot try other keys.
        """
        if credential_count > 1:
            lib_logger.debug(
                "Skipping provider-wide rate-limit cooldown for %s (%d credentials)",
                provider,
                credential_count,
            )
            return
    ...
    async def close(self):
        ...
        for _idx, oc in list(getattr(self, "_ollama_proxy_clients", {}).items()):
            try:
                await oc.aclose()
            except Exception:
                pass
        self._ollama_proxy_clients = {}
    ...
    async def _safe_streaming_wrapper(
        ...
        *,
        stream_provider: str = "",
        documind_document_complete: bool = False,
        ...
        try:
            while True:
                ...
                try:
                    ...
                    if stream_provider == "ollama_cloud" and self.documind_policy:
                        idx = self._ollama_cloud_cred_to_index.get(key)
                        if idx is not None:
                            self.documind_policy.on_upstream_success(
                                idx, {}, documind_document_complete
                            )
                            self._documind_last_success_index = idx
    ...
    async def _execute_with_retry(
        ...
        **kwargs,
        ...
        documind_pref, documind_busy, documind_doc_complete = (
            parse_documind_request_headers(
                getattr(request, "headers", None) if request else None
            )
        )
        ...
        if not (
            ...
            and getattr(self, "documind_policy", None) is not None
        ):
        ...
        if provider == "ollama_cloud" and self.documind_policy:
            credentials_for_provider = order_credentials_documind(
                credentials_for_provider,
                self._ollama_cloud_cred_to_index,
                self.documind_policy,
                documind_pref,
                documind_busy,
                self._documind_last_success_index,
            )
                        ...
                        error_accumulator.record_error(
                            ...
                            ClassifiedError(
                                ...
                                original_exception=RuntimeError(
                                    "proxy_provider_cooldown_exceeds_budget"
                                ),
                                status_code=429,
                                retry_after=int(max(1, min(remaining_cooldown, 86400))),
                            ),
                            (
                                f"Provider {provider} cooldown ({remaining_cooldown:.0f}s) exceeds "
                                f"request budget ({remaining_budget:.0f}s); increase proxy timeout or wait"
                            ),
                        )
                        break
                ...
                if provider_plugin and provider_plugin.has_custom_logic():
                    ...
                    for attempt in range(self.max_retries):
                        try:
                            ...
                            self._documind_on_success(
                                provider,
                                current_cred,
                                response,
                                documind_doc_complete,
                            )
                            ...
                            if classified_error.error_type == "rate_limit":
                                await self._apply_provider_rate_limit_cooldown_if_needed(
                                    provider=provider,
                                    classified_error=classified_error,
                                    credential_count=len(credentials_for_provider),
                                )

                            if (
                                classified_error.error_type == "rate_limit"
                                or classified_error.status_code == 429
                            ):
                                self._documind_on_429(
                                    provider,
                                    current_cred,
                                    e,
                                    error_message,
                    ...
                    if provider == "ollama_cloud":
                        self._merge_ollama_cloud_litellm_outbound(
                            litellm_kwargs, current_cred
                        )
                        ...
                        for attempt in range(self.max_retries):
                            try:
                                ...
                                stream_generator = self._safe_streaming_wrapper(
                                    ...
                                    documind_document_complete=documind_doc_complete,
                                )
