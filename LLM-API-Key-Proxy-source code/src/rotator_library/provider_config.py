...
def normalize_ollama_cloud_model_name(model_name: str) -> str:
    """
    Map names to IDs that exist on Ollama Cloud's OpenAI endpoint (https://ollama.com/v1).

    Qwen3.5 on Cloud uses the :cloud tag (https://ollama.com/library/qwen3.5:cloud).
    Bare ``qwen3.5`` or local-style tags like ``qwen3.5:9b`` often return HTTP 404 on Cloud.
    """
    if not model_name:
        return model_name

    raw_map = os.getenv("OLLAMA_CLOUD_MODEL_MAP", "").strip()
    if raw_map:
        try:
            custom = json.loads(raw_map)
            if isinstance(custom, dict):
                v = custom.get(model_name)
                if v is None:
                    v = custom.get(model_name.lower())
                if isinstance(v, str) and v.strip():
                    return v.strip()
        except (json.JSONDecodeError, TypeError):
            lib_logger.warning("OLLAMA_CLOUD_MODEL_MAP is not valid JSON; ignored")
    ...
    return defaults.get(low, model_name)
    ...
    def _load_api_bases(self) -> None:
        ...
        for key, value in os.environ.items():
            if key.endswith("_API_BASE") and value:
                ...
                base = value.rstrip("/")
                # Ollama Cloud OpenAI-compatible chat is under .../v1, not .../ai/v1
                # (the latter returns the marketing site's HTML 404).
                if provider == "ollama_cloud" and base.endswith("/ai/v1"):
                    fixed = base[: -len("/ai/v1")] + "/v1"
                    lib_logger.warning(
                        "%s uses deprecated path /ai/v1 (HTML 404); using %s",
                        key,
                        fixed,
                    )
                    base = fixed
                # Keys from ollama.com/settings/keys authenticate on ollama.com, not api.ollama.com
                # (the latter often returns 401 for the same Bearer token).
                if provider == "ollama_cloud" and "//api.ollama.com" in base:
                    fixed = base.replace("//api.ollama.com", "//ollama.com", 1)
                    lib_logger.warning(
                        "%s uses api.ollama.com; cloud API keys are for ollama.com. Using %s",
                        key,
                        fixed,
                    )
                    base = fixed
                self._api_bases[provider] = base
                ...
                if provider not in KNOWN_PROVIDERS:
                    ...
                    lib_logger.info(
                        f"Detected API base override for {provider}: {base}"
                    )
    ...
    def convert_for_litellm(self, **kwargs) -> Dict[str, Any]:
        ...
        if provider in KNOWN_PROVIDERS:
            ...
            if provider == "ollama_cloud":
                new_name = normalize_ollama_cloud_model_name(model_name)
                if new_name != model_name:
                    lib_logger.info(
                        "Ollama Cloud model normalized: %s -> %s", model_name, new_name
                    )
                model_name = new_name
