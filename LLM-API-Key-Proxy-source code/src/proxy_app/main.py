...
# Documind: upstream API keys synced from app settings (written by gui/server.js)
_documind_sync = _root_dir / "documind_synced_keys.env"
if _documind_sync.is_file():
    load_dotenv(_documind_sync, override=True)
...
for key, value in os.environ.items():
    if "_API_KEY" in key and key != "PROXY_API_KEY":
        ...
        if provider not in api_keys:
            api_keys[provider] = []
        api_keys[provider].append(value.strip())

# Deterministic Ollama Cloud key order (matches Documind slot indices)
from rotator_library.documind_ollama_policy import collect_ollama_cloud_keys_ordered

_ollama_cloud_keys = collect_ollama_cloud_keys_ordered()
if _ollama_cloud_keys:
    api_keys["ollama_cloud"] = _ollama_cloud_keys
...
async def lifespan(app: FastAPI):
    ...
    # Ollama Cloud: parallel Documind work often 429s many keys at once; each gets a
    # ~60s transient cooldown. A 30s budget cannot wait for eligibility → false 500s.
    if api_keys.get("ollama_cloud"):
        # Default 100s: survives a full 60s key cooldown plus LLM latency; 75s was tight when
        # many keys cool at once. Set lower for faster fail, or GLOBAL_TIMEOUT explicitly.
        floor_raw = os.getenv("OLLAMA_CLOUD_GLOBAL_TIMEOUT_FLOOR", "100")
        try:
            ollama_floor = int(floor_raw)
        except ValueError:
            ollama_floor = 100
        if ollama_floor > 0 and global_timeout < ollama_floor:
            logging.info(
                "GLOBAL_TIMEOUT raised %ds → %ds (ollama_cloud keys; "
                "OLLAMA_CLOUD_GLOBAL_TIMEOUT_FLOOR or GLOBAL_TIMEOUT to tune).",
                global_timeout,
                ollama_floor,
            )
            global_timeout = ollama_floor
    ...
    client.import_documind_legacy_if_present()
...
def _strip_documind_routing_keys_from_body(body: Any) -> None:
    """Documind routing hints must be HTTP headers; drop mistaken JSON copies before LiteLLM."""
    if not isinstance(body, dict):
        return
    for k in list(body.keys()):
        if str(k).lower().startswith("x-documind"):
            body.pop(k, None)
...
def _proxy_error_response_http_status(body: dict) -> int:
    """Map RotatingClient error dict (OpenAI-style ``error`` object) to an HTTP status."""
    err = body.get("error") or {}
    et = err.get("type", "")
    if et == "proxy_timeout":
        return 504
    details = err.get("details") or {}
    for ae in details.get("abnormal_errors") or []:
        sc = ae.get("status_code")
        if sc in (400, 401, 403, 404):
            return int(sc)
    if et == "proxy_all_credentials_exhausted":
        return 429
    return 503
...
async def chat_completions(
    ...
    try:
        ...
        _strip_documind_routing_keys_from_body(request_data)
        ...
        if is_streaming:
            ...
            if response is None:
                raise HTTPException(
                    status_code=503,
                    detail="Proxy could not complete the request (no error details).",
                )
            if isinstance(response, dict) and response.get("error"):
                status_code = _proxy_error_response_http_status(response)
                if raw_logger:
...
@app.get("/v1/documind/key-cache")
async def documind_key_cache(
    client: RotatingClient = Depends(get_rotating_client),
    _=Depends(verify_api_key),
):
    pol = getattr(client, "documind_policy", None)
    if not pol:
        raise HTTPException(
            status_code=404,
            detail="No ollama_cloud credentials — Documind policy inactive.",
        )
    return {
        "cache": pol.key_cache_snapshot(),
        "quotaUsage": pol.snapshot_gui(use_cloud_ref=True),
    }


@app.post("/v1/documind/key-cache/clear")
async def documind_key_cache_clear(
    client: RotatingClient = Depends(get_rotating_client),
    _=Depends(verify_api_key),
):
    pol = getattr(client, "documind_policy", None)
    if not pol:
        raise HTTPException(status_code=404, detail="Documind policy inactive.")
    pol.clear_key_cache()
    return {"success": True}


@app.get("/v1/documind/quota-tracking")
async def documind_quota_tracking_get(
    client: RotatingClient = Depends(get_rotating_client),
    _=Depends(verify_api_key),
):
    from rotator_library.documind_ollama_policy import OLLAMA_QUOTA_KEY_COUNT

    pol = getattr(client, "documind_policy", None)
    if not pol:
        raise HTTPException(status_code=404, detail="Documind policy inactive.")
    snap = pol.snapshot_gui(use_cloud_ref=True)
    tk = snap.get("trackedKeys") or {}
    tracked_keys = [tk.get(str(i), True) is not False for i in range(OLLAMA_QUOTA_KEY_COUNT)]
    return {"trackedKeys": tracked_keys}


@app.post("/v1/documind/quota-tracking")
async def documind_quota_tracking_post(
    request: Request,
    client: RotatingClient = Depends(get_rotating_client),
    _=Depends(verify_api_key),
):
    ...
    tracked = body.get("trackedKeys")
    if not isinstance(tracked, list) or len(tracked) != OLLAMA_QUOTA_KEY_COUNT:
        raise HTTPException(
            status_code=400,
            detail=f"Body must include trackedKeys: boolean[{OLLAMA_QUOTA_KEY_COUNT}]",
        )
    pol.set_tracked_keys([bool(x) for x in tracked])
    return {"success": True}


@app.get("/v1/documind/ollama-usage")
async def documind_ollama_usage(
    client: RotatingClient = Depends(get_rotating_client),
    _=Depends(verify_api_key),
):
    from rotator_library.documind_ollama_policy import (
        OLLAMA_QUOTA_KEY_COUNT,
        parse_ollama_session_reset_at_ms,
    )

    keys = client.all_credentials.get("ollama_cloud") or []
    base = (os.getenv("OLLAMA_CLOUD_API_BASE") or "https://ollama.com/v1").rstrip("/")
    if base.endswith("/v1"):
        root = base[: -len("/v1")]
    else:
        root = base
    ...
    async with httpx.AsyncClient(timeout=5.0) as ac:
        for i in range(min(len(keys), OLLAMA_QUOTA_KEY_COUNT)):
            usage[i] = {"session": None, "weekly": None, "error": None}
            try:
                r = await ac.get(
                    f"{root}/api/tags",
                ...
                h = r.headers
                session_used = h.get("x-ollama-session-used") or h.get("x-usage-session")
                session_limit = h.get("x-ollama-session-limit") or h.get(
                    "x-usage-session-limit"
                )
                weekly_used = h.get("x-ollama-weekly-used") or h.get("x-usage-weekly")
                weekly_limit = h.get("x-ollama-weekly-limit") or h.get(
                    "x-usage-weekly-limit"
                )
                if session_used is not None or session_limit is not None:
                    usage[i]["session"] = {
                        "used": float(session_used) if session_used else None,
                        "limit": float(session_limit) if session_limit else None,
                    }
                if weekly_used is not None or weekly_limit is not None:
                    usage[i]["weekly"] = {
                        "used": float(weekly_used) if weekly_used else None,
                        "limit": float(weekly_limit) if weekly_limit else None,
                    }
                if r.status_code == 429:
                    usage[i]["error"] = "Rate limited"
                ra = parse_ollama_session_reset_at_ms(dict(h))
                if ra is not None:
                    usage[i]["sessionResetAtMs"] = ra
            except Exception as ex:
                usage[i]["error"] = str(ex) or "Request failed"
    return {"usage": usage, "keysCount": len(keys)}
